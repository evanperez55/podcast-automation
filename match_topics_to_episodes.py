...
class TopicMatcher:
    ...
    def __init__(self):
        ...
        print("[OK] Topic matcher ready with Ollama (FREE)")
    ...
    def match_topics_batch(
        self, topics: List[Dict], episodes: List[Dict], batch_size: int = 50
    ) -> List[Dict]:
        """
        ...
        print(
            f"[INFO] Processing {len(topics)} topics in {total_batches} batches of {batch_size}"
        )
        ...
        for batch_num in range(total_batches):
            ...
            try:
                batch_matches = self._match_batch_with_claude(
                    batch_topics, episode_context, start_idx
                )
                all_matches.extend(batch_matches)
                print(
                    f"[OK] Batch {batch_num + 1} complete: {len(batch_matches)} matches"
                )
    ...
    def _match_batch_with_claude(
        self, topics: List[Dict], episode_context: str, batch_start_idx: int
    ) -> List[Dict]:
...
Return the JSON array now:"""
    ...
    def generate_analysis_report(self, matches: List[Dict]) -> Dict:
        ...
        report = {
            ...
            "match_percentage": round(len(matched) / total_topics * 100, 2)
            if total_topics > 0
            else 0,
    ...
    if report["potential_duplicates"]:
        ...
        for dup in report["potential_duplicates"][:5]:
            ...
            for sim in dup["similar_topics"]:
                print(
                    f"  - Similar to #{sim['topic_id']} ({sim['similarity'] * 100:.0f}% match): {sim['text'][:60]}..."
                )
            print()
