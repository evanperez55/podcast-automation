...
class NotionTopicManager:
    ...
    def create_topic(self, topic_data: Dict) -> Dict:
        ...
        """
        # Build properties
        properties = {"Name": {"title": [{"text": {"content": topic_data["topic"]}}]}}

        # Add score if provided
        if "score" in topic_data and topic_data["score"] is not None:
            properties["Score"] = {"number": float(topic_data["score"])}

        # Add category if provided
        if "category" in topic_data and topic_data["category"]:
            properties["Category"] = {"select": {"name": topic_data["category"]}}
            ...
            children.append(
                {
                    ...
                    "paragraph": {
                        "rich_text": [
                            {"type": "text", "text": {"content": topic_data["notes"]}}
                        ]
                    },
                }
            )
    ...
    def bulk_create_topics(self, topics: List[Dict]) -> List[Dict]:
        ...
        for i, topic in enumerate(topics, 1):
            ...
            if topic_text in existing_topics:
                ...
                if i % 10 == 0:
                    print(
                        f"[INFO] Processed {i}/{total} topics (skipped {len(skipped)} duplicates)..."
                    )
                continue
            ...
            try:
                ...
                if i % 10 == 0:
                    print(
                        f"[INFO] Created {len(created)}/{total} topics (skipped {len(skipped)} duplicates)..."
                    )
        ...
        print(
            f"[OK] Created {len(created)} new topics, skipped {len(skipped)} duplicates"
        )
        return created

    def query_database(
        self, filter_params: Optional[Dict] = None, sorts: Optional[List[Dict]] = None
    ) -> List[Dict]:
        """
    ...
    def get_topics_by_status(self, status: str) -> List[Dict]:
        ...
        filter_params = {"property": "Status", "select": {"equals": status}}
    ...
    def update_topic_status(self, page_id: str, new_status: str) -> Dict:
        ...
        data = {"properties": {"Status": {"select": {"name": new_status}}}}
    ...
    def mark_topic_as_discussed(self, page_id: str, episode_number: int) -> Dict:
