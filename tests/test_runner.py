...
# ---------------------------------------------------------------------------
# Per-step failure notifications and duration tracking (_run_pipeline)
# ---------------------------------------------------------------------------
class TestRunPipelineStepNotifications:
    """Tests for per-step failure notifications in _run_pipeline."""

    @patch("pipeline.runner.run_distribute")
    @patch("pipeline.runner.run_video")
    @patch("pipeline.runner.run_analysis")
    @patch("pipeline.runner.run_ingest")
    @patch("pipeline.runner._run_process_audio")
    @patch("pipeline.runner._run_transcribe")
    @patch("pipeline.runner._init_components")
    @patch("pipeline.runner._acquire_pipeline_lock", return_value=True)
    @patch("pipeline.runner._release_pipeline_lock")
    def test_step_failure_sends_notification(
        self,
        mock_release,
        mock_acquire,
        mock_init,
        ...
        """When run_ingest raises, notify_failure is called with step='ingest' and exception re-raises."""
        from pipeline.runner import run

        mock_notifier = Mock()
        mock_notifier.enabled = True

        mock_init.return_value = {"notifier": mock_notifier}
        ...
        mock_notifier.notify_failure.assert_called_once()
        call_kwargs = mock_notifier.notify_failure.call_args
        assert (
            call_kwargs.kwargs.get("step") == "ingest"
            or call_kwargs[1].get("step") == "ingest"
        )
    ...
    @patch("pipeline.runner._acquire_pipeline_lock", return_value=True)
    @patch("pipeline.runner._release_pipeline_lock")
    def test_step_failure_reraises_exception(
        self,
        mock_release,
        mock_acquire,
        mock_init,
        ...
        """Exception from a failing step is re-raised after notification."""
        from pipeline.runner import run
    ...
    @patch("pipeline.runner._acquire_pipeline_lock", return_value=True)
    @patch("pipeline.runner._release_pipeline_lock")
    def test_step_failure_notifier_disabled_no_crash(
        self,
        mock_release,
        mock_acquire,
        mock_init,
        ...
        """When notifier is disabled, step failure still re-raises without crashing."""
        from pipeline.runner import run

        mock_notifier = Mock()
        mock_notifier.enabled = False
        mock_init.return_value = {"notifier": mock_notifier}
        mock_ingest.side_effect = Exception("Fail silently")
        ...
        with pytest.raises(Exception, match="Fail silently"):
            run(args)
    ...
    @patch("pipeline.runner._acquire_pipeline_lock", return_value=True)
    @patch("pipeline.runner._release_pipeline_lock")
    def test_pipeline_success_includes_duration(
        self,
        mock_release,
        mock_acquire,
        mock_init,
        ...
        """Successful pipeline run includes duration_seconds in results dict."""
        from pipeline.runner import run
        from pipeline.context import PipelineContext

        # time.time() returns 1000.0 first call (start), 1754.0 at end
        mock_time.time.side_effect = [1000.0, 1754.0]
        ...
        ctx.transcript_data = SAMPLE_TRANSCRIPT
        ctx.transcript_path = tmp_path / "transcript.json"
        ctx.censored_audio = censored
        ctx.mp3_path = mp3_path
        ctx.clip_paths = []
        ctx.video_clip_paths = []
        ctx.finished_path = None
        ctx.uploaded_clip_paths = []
        ctx.thumbnail_path = None
        ctx.full_episode_video_path = None

        mock_notifier = Mock()
        mock_notifier.enabled = False
        mock_init.return_value = {"notifier": mock_notifier}
        mock_ingest.return_value = ctx
        mock_transcribe.return_value = ctx
        mock_analysis.return_value = ctx
        mock_process_audio.return_value = ctx
        mock_video.return_value = ctx
        mock_distribute.return_value = ctx
        ...
        assert results["duration_seconds"] == pytest.approx(754.0)
